import React from 'react'
import { DemoBlock } from 'demos'
import { Image, Space } from 'components'
import './demo2.less'

const demoSrc =
  'https://lzd-img-global.slatic.net/g/tps/tfs/TB12FYKn21TBuNjy0FjXXajyXXa-1920-1080.jpg'
const demoSrc2 =
  'https://lzd-img-global.slatic.net/g/tps/imgextra/i4/O1CN01JaAJZ41nE9cWJN9DV_!!6000000005057-0-tps-750-422.jpg'

const placeholder = 'data:image/png;base64,';
export default () => {
  return (
    <>
      <DemoBlock title='基础用法'>
        <Image type="arise" src={demoSrc} />
      </DemoBlock>

      <DemoBlock title='加载失败'>
        <Image type="arise" src='/404' width={100} height={100} />
      </DemoBlock>
      <DemoBlock title='加载失败 + 图片白色底占位'>
        <div style={{ backgroundColor: '#f7f7f7',  padding: 10  }}>
        <Image type="arise" placeholderType="light" src='/404' width={80} height={80} />
        </div>
      </DemoBlock>
      <DemoBlock title='加载失败 + 修改占位图大小'>
        <Image type="arise" className="custom-placehoder" src='/404' width={100} height={100} />
      </DemoBlock>
      <DemoBlock title='多种填充模式'>
        <Space wrap>
          <Image type="arise" src={demoSrc} width={100} height={100} fit='fill' />
          <Image type="arise" src={demoSrc} width={100} height={100} fit='contain' />
          <Image type="arise" src={demoSrc} width={100} height={100} fit='cover' />
          <Image type="arise" src={demoSrc} width={100} height={100} fit='scale-down' />
          <Image type="arise" src={demoSrc} width={100} height={100} fit='none' />
        </Space>
      </DemoBlock>
    </>
  )
}
